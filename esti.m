function  [etahatall,sigmaeta,gamhat,sigmagam]=esti(n,px,pz,...
...
while error>0.00001 && iter<30
    for igrid=1:ngrid
        ...
        Beta(:,igrid)=Deta(:,:,igrid)'*Psihat*(Rhat(:,igrid)-F1hat(:,igrid));
        Bgam(:,igrid)=Dgam(:,:,igrid)'*Psihat*(Rhat(:,igrid)-F1hat(:,igrid));
        
        
        Cetaeta(:,:,igrid)=Deta(:,:,igrid)'*Psihat*Deta(:,:,igrid);
        Cgameta(:,:,igrid)=Dgam(:,:,igrid)'*Psihat*Deta(:,:,igrid);
        Cgamgam(:,:,igrid)=Dgam(:,:,igrid)'*Psihat*Dgam(:,:,igrid);
...
for sgrid=1:ngrid
    for igrid=1:ngrid
        qetahat(:,sgrid,igrid)=Deta(:,:,igrid)'*Psihat*(Rhat(:,igrid).*(time>=times(sgrid)))/n;
        qgamhat(:,sgrid,igrid)=Dgam(:,:,igrid)'*Psihat*(Rhat(:,igrid).*(time>=times(sgrid)))/n;
...
for i=1:n
    for igrid=1:ngrid
        geta(:,:,igrid)=geta(:,:,igrid)-rselec(i)*Deta(i,:,igrid)'...
            *(Rhat(i,igrid)-F1hat(i,igrid))/shat(i)^2*dpi(:,i)'/n;
        ggam(:,:)=ggam(:,:)-rselec(i)*Dgam(i,:,igrid)'...
            *(Rhat(i,igrid)-F1hat(i,igrid))/shat(i)^2*dpi(:,i)'/n*dt;
    end
end
...
for i=1:n
    for igrid=1:ngrid
        Betaihat(:,i,igrid)=Deta(i,:,igrid)'*w_est(i)*(Rhat(i,igrid)-F1hat(i,igrid));
        Bgamihat(:,i,igrid)=Dgam(i,:,igrid)'*w_est(i)*(Rhat(i,igrid)-F1hat(i,igrid));
...
Wgamihat=BKBihatint+PhiGihat+Phipi;
for i=1:n
